...
verify(){
	echo "exit" | ${SQLPLUS} | grep Connected > /dev/null
	if [ $? -eq 0 ];
	then
...
apex_epg_config(){
	...
	$SQLPLUS @apex_epg_config /u01/app/oracle < /dev/null
}
